...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(runListResponse: RunListResponse) = apply {
            missionId = runListResponse.missionId
            runId = runListResponse.runId
            startedAt = runListResponse.startedAt
            status = runListResponse.status
            updatedAt = runListResponse.updatedAt
            error = runListResponse.error
            finishedAt = runListResponse.finishedAt
            input = runListResponse.input
            metadata = runListResponse.metadata
            resultPayload = runListResponse.resultPayload
            resultSummary = runListResponse.resultSummary
    ...
    override fun toString() =
        "RunListResponse{missionId=$missionId, runId=$runId, startedAt=$startedAt, status=$status, updatedAt=$updatedAt, error=$error, finishedAt=$finishedAt, input=$input, metadata=$metadata, resultPayload=$resultPayload, resultSummary=$resultSummary, additionalProperties=$additionalProperties}"
}
