...
@JsonDeserialize(using = PortoutEvent.Deserializer::class)
@JsonSerialize(using = PortoutEvent.Serializer::class)
class PortoutEvent
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = portoutEvent.accept(new PortoutEvent.Visitor<Optional<String>>() {
     *     @Override
     *     public Optional<String> visitWebhookPortoutStatusChanged(WebhookPortoutStatusChanged webhookPortoutStatusChanged) {
     *         return Optional.of(webhookPortoutStatusChanged.toString());
     *     }
     *
     *     // ...
     *
     ...
     *   expected type.
     */
    fun validate(): PortoutEvent = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun toString(): String =
        when {
            webhookPortoutStatusChanged != null ->
                "PortoutEvent{webhookPortoutStatusChanged=$webhookPortoutStatusChanged}"
            webhookPortoutNewComment != null ->
                "PortoutEvent{webhookPortoutNewComment=$webhookPortoutNewComment}"
            webhookPortoutFocDateChanged != null ->
                "PortoutEvent{webhookPortoutFocDateChanged=$webhookPortoutFocDateChanged}"
            _json != null -> "PortoutEvent{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid PortoutEvent")
        }
    ...
    /**
     * An interface that defines how to map each variant of [PortoutEvent] to a value of type [T].
     */
    interface Visitor<out T> {
        ...
        /**
         * Maps an unknown variant of [PortoutEvent] to a value of type [T].
         *
         * An instance of [PortoutEvent] can contain an unknown variant if it was deserialized from
        ...
        fun unknown(json: JsonValue?): T {
            throw TelnyxInvalidDataException("Unknown PortoutEvent: $json")
        }
    }

    internal class Deserializer : BaseDeserializer<PortoutEvent>(PortoutEvent::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): PortoutEvent {
            ...
            val bestMatches =
                sequenceOf(
                        tryDeserialize(node, jacksonTypeRef<WebhookPortoutStatusChanged>())?.let {
                            PortoutEvent(webhookPortoutStatusChanged = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<WebhookPortoutNewComment>())?.let {
                            PortoutEvent(webhookPortoutNewComment = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<WebhookPortoutFocDateChanged>())?.let {
                            PortoutEvent(webhookPortoutFocDateChanged = it, _json = json)
                        },
                    )
                    .filterNotNull()
    ...
    internal class Serializer : BaseSerializer<PortoutEvent>(PortoutEvent::class) {
