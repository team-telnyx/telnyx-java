...
/**
 * A CloudFS filesystem as returned by get, update, and delete. `meta_url` omits the credential and
 * there is no `meta_token` field — the token is only returned by create and rotate-meta-token.
 */
class CloudfsFilesystemDetail
...
private constructor(
    ...
    /**
     * Explanation of the most recent failed lifecycle action. Present only when the filesystem is
     * in a `failed` state.
     *
     * @throws TelnyxInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
    ...
    companion object {
        ...
        /** Returns a mutable builder for constructing an instance of [CloudfsFilesystemDetail]. */
        @JvmStatic fun builder() = Builder()
    }

    /** A builder for [CloudfsFilesystemDetail]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(cloudfsFilesystemDetail: CloudfsFilesystemDetail) = apply {
            id = cloudfsFilesystemDetail.id
            createdAt = cloudfsFilesystemDetail.createdAt
            error = cloudfsFilesystemDetail.error
            metaUrl = cloudfsFilesystemDetail.metaUrl
            name = cloudfsFilesystemDetail.name
            recordType = cloudfsFilesystemDetail.recordType
            region = cloudfsFilesystemDetail.region
            s3Bucket = cloudfsFilesystemDetail.s3Bucket
            s3Endpoint = cloudfsFilesystemDetail.s3Endpoint
            status = cloudfsFilesystemDetail.status
            updatedAt = cloudfsFilesystemDetail.updatedAt
            additionalProperties = cloudfsFilesystemDetail.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [CloudfsFilesystemDetail].
         *
         * Further updates to this [Builder] will not mutate the returned instance.
         */
        fun build(): CloudfsFilesystemDetail =
     ...
     *   expected type.
     */
    fun validate(): CloudfsFilesystemDetail = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is CloudfsFilesystemDetail &&
            id == other.id &&
    ...
    override fun toString() =
        "CloudfsFilesystemDetail{id=$id, createdAt=$createdAt, error=$error, metaUrl=$metaUrl, name=$name, recordType=$recordType, region=$region, s3Bucket=$s3Bucket, s3Endpoint=$s3Endpoint, status=$status, updatedAt=$updatedAt, additionalProperties=$additionalProperties}"
}
