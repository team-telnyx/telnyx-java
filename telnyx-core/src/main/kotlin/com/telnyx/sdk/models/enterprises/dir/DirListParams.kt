 ...
 * case-insensitive partial match on `display_name` (`search=`). Sortable by any of `created_at`,
 * `updated_at`, `display_name`, `status`, `submitted_at`, `verified_at`, `expiring_at` (prefix `-`
 * for descending; default `-created_at`). Supports the renewal-window filters
...
private constructor(
    ...
    override fun toString() =
        "DirListParams{enterpriseId=$enterpriseId, filterExpiringAtGte=$filterExpiringAtGte, filterExpiringAtLte=$filterExpiringAtLte, filterExpiringWithinDays=$filterExpiringWithinDays, pageNumber=$pageNumber, pageSize=$pageSize, search=$search, sort=$sort, status=$status, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
