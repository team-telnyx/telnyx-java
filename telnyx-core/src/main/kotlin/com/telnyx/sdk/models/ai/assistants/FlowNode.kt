...
private constructor(
    ...
    /**
     * Full tool definitions for this node, resolved from `shared_tool_ids` server-side. Populated
     * on responses so clients can render the flow without a follow-up fetch per shared tool.
     * Ignored on input — set `shared_tool_ids` to configure a node's tools.
     *
     * @throws TelnyxInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
     */
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [FlowNode].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(flowNode: FlowNode) = apply {
            id = flowNode.id
            instructions = flowNode.instructions
            externalLlm = flowNode.externalLlm
            instructionsMode = flowNode.instructionsMode
            llmApiKeyRef = flowNode.llmApiKeyRef
            model = flowNode.model
            name = flowNode.name
            position = flowNode.position
            sharedToolIds = flowNode.sharedToolIds.map { it.toMutableList() }
            tools = flowNode.tools.map { it.toMutableList() }
            toolsMode = flowNode.toolsMode
            transcription = flowNode.transcription
            type = flowNode.type
            voiceSettings = flowNode.voiceSettings
            additionalProperties = flowNode.additionalProperties.toMutableMap()
        }
         ...
         * Populated on responses so clients can render the flow without a follow-up fetch per
         * shared tool. Ignored on input — set `shared_tool_ids` to configure a node's tools.
         */
         ...
         *
         * You should usually call [Builder.tools] with a well-typed `List<List<AssistantTool>>`
         * value instead. This method is primarily for setting the field to an undocumented or not
         * yet supported value.
         */
        ...
        /**
         * Returns an immutable instance of [FlowNode].
         *
    ...
    internal fun validity(): Int =
        (if (id.asKnown().isPresent) 1 else 0) +
            ...
            (tools.asKnown().getOrNull()?.sumOf { it.sumOf { it.validity().toInt() }.toInt() }
                ?: 0) +
    ...
    override fun toString() =
        "FlowNode{id=$id, instructions=$instructions, externalLlm=$externalLlm, instructionsMode=$instructionsMode, llmApiKeyRef=$llmApiKeyRef, model=$model, name=$name, position=$position, sharedToolIds=$sharedToolIds, tools=$tools, toolsMode=$toolsMode, transcription=$transcription, type=$type, voiceSettings=$voiceSettings, additionalProperties=$additionalProperties}"
}
