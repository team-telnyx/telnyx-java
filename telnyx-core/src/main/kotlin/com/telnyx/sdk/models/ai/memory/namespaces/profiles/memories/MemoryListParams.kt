 ...
 * being written to. Each memory names the `source_id` it was extracted from, or null for a memory
 * derived from other memories -- which can read almost the same as the fact it restates. A
 * `source_id` narrows the listing to the memories extracted from that source, and a `session_id` to
 * those extracted from the session, which is the same thing named another way; pass one or the
 * other. Neither is everything the source led to: a memory derived from several sources belongs to
 * no single one and appears only in the unfiltered listing. A memory written while the listing is
 * paged shifts the pages after it, so an entry can be repeated or missed at a page boundary.
 */
...
private constructor(
    ...
    /**
     * Narrows the listing to the memories extracted from one source, a remembered fact as well as a
     * session. Pass this or `session_id`, not both.
     */
    ...
    class Builder internal constructor() {
        ...
        internal fun from(memoryListParams: MemoryListParams) = apply {
            namespace = memoryListParams.namespace
            profileId = memoryListParams.profileId
            pageNumber = memoryListParams.pageNumber
            pageSize = memoryListParams.pageSize
            sessionId = memoryListParams.sessionId
            sourceId = memoryListParams.sourceId
        ...
        /**
         * Narrows the listing to the memories extracted from one source, a remembered fact as well
         * as a session. Pass this or `session_id`, not both.
         */
    ...
    override fun toString() =
        "MemoryListParams{namespace=$namespace, profileId=$profileId, pageNumber=$pageNumber, pageSize=$pageSize, sessionId=$sessionId, sourceId=$sourceId, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
