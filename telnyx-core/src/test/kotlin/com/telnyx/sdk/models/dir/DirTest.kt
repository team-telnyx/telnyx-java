// File generated from our OpenAPI spec by Stainless.

package com.telnyx.sdk.models.dir

import com.fasterxml.jackson.module.kotlin.jacksonTypeRef
import com.telnyx.sdk.core.jsonMapper
import com.telnyx.sdk.models.dir.phonenumbers.RejectionReason
import java.time.OffsetDateTime
import kotlin.jvm.optionals.getOrNull
import org.assertj.core.api.Assertions.assertThat
import org.junit.jupiter.api.Test

internal class DirTest {

    @Test
    fun create() {
        val dir =
            Dir.builder()
                .id("16635d38-75a6-4481-82e8-69af60e05011")
                .authorizerEmail("sam@acmeplumbing.example.com")
                .authorizerName("Sam Owner")
                .addCallReason(
                    Dir.CallReason.builder()
                        .createdAt(OffsetDateTime.parse("2026-04-26T18:06:51.940749Z"))
                        .reason("Appointment reminders")
                        .build()
                )
                .certifyBrandIsAccurate(true)
                .certifyIpOwnership(true)
                .certifyNoShaftContent(true)
                .createdAt(OffsetDateTime.parse("2026-04-26T18:06:51.940749Z"))
                .displayName("Acme Plumbing")
                .addDocument(
                    Document.builder()
                        .documentId("2a7e8337-e803-4057-a4ae-26c40eb0bc6c")
                        .documentType(Document.DocumentType.BUSINESS_REGISTRATION)
                        .description("Certificate of incorporation.")
                        .build()
                )
                .enterpriseId("4a6192a4-573d-446d-b3ce-aff9117272a6")
                .expiringAt(OffsetDateTime.parse("2027-04-26T18:07:29.537981Z"))
                .logoUrl("https://acmeplumbing.example.com/logo-256.bmp")
                .rejectedAt(OffsetDateTime.parse("2019-12-27T18:11:19.117Z"))
                .addRejectionReason(
                    RejectionReason.builder()
                        .code("documentation_incomplete")
                        .detail("Provided documents do not establish business identity.")
                        .message("Please re-upload a clearer scan of the certificate.")
                        .title("Documentation incomplete")
                        .build()
                )
                .reselling(false)
                .status(DirStatus.DRAFT)
                .submittedAt(OffsetDateTime.parse("2026-04-26T18:07:03.716411Z"))
                .updatedAt(OffsetDateTime.parse("2026-04-26T18:09:24.785211Z"))
                .verifiedAt(OffsetDateTime.parse("2026-04-26T18:07:29.537926Z"))
                .build()

        assertThat(dir.id()).contains("16635d38-75a6-4481-82e8-69af60e05011")
        assertThat(dir.authorizerEmail()).contains("sam@acmeplumbing.example.com")
        assertThat(dir.authorizerName()).contains("Sam Owner")
        assertThat(dir.callReasons().getOrNull())
            .containsExactly(
                Dir.CallReason.builder()
                    .createdAt(OffsetDateTime.parse("2026-04-26T18:06:51.940749Z"))
                    .reason("Appointment reminders")
                    .build()
            )
        assertThat(dir.certifyBrandIsAccurate()).contains(true)
        assertThat(dir.certifyIpOwnership()).contains(true)
        assertThat(dir.certifyNoShaftContent()).contains(true)
        assertThat(dir.createdAt()).contains(OffsetDateTime.parse("2026-04-26T18:06:51.940749Z"))
        assertThat(dir.displayName()).contains("Acme Plumbing")
        assertThat(dir.documents().getOrNull())
            .containsExactly(
                Document.builder()
                    .documentId("2a7e8337-e803-4057-a4ae-26c40eb0bc6c")
                    .documentType(Document.DocumentType.BUSINESS_REGISTRATION)
                    .description("Certificate of incorporation.")
                    .build()
            )
        assertThat(dir.enterpriseId()).contains("4a6192a4-573d-446d-b3ce-aff9117272a6")
        assertThat(dir.expiringAt()).contains(OffsetDateTime.parse("2027-04-26T18:07:29.537981Z"))
        assertThat(dir.logoUrl()).contains("https://acmeplumbing.example.com/logo-256.bmp")
        assertThat(dir.rejectedAt()).contains(OffsetDateTime.parse("2019-12-27T18:11:19.117Z"))
        assertThat(dir.rejectionReasons().getOrNull())
            .containsExactly(
                RejectionReason.builder()
                    .code("documentation_incomplete")
                    .detail("Provided documents do not establish business identity.")
                    .message("Please re-upload a clearer scan of the certificate.")
                    .title("Documentation incomplete")
                    .build()
            )
        assertThat(dir.reselling()).contains(false)
        assertThat(dir.status()).contains(DirStatus.DRAFT)
        assertThat(dir.submittedAt()).contains(OffsetDateTime.parse("2026-04-26T18:07:03.716411Z"))
        assertThat(dir.updatedAt()).contains(OffsetDateTime.parse("2026-04-26T18:09:24.785211Z"))
        assertThat(dir.verifiedAt()).contains(OffsetDateTime.parse("2026-04-26T18:07:29.537926Z"))
    }

    @Test
    fun roundtrip() {
        val jsonMapper = jsonMapper()
        val dir =
            Dir.builder()
                .id("16635d38-75a6-4481-82e8-69af60e05011")
                .authorizerEmail("sam@acmeplumbing.example.com")
                .authorizerName("Sam Owner")
                .addCallReason(
                    Dir.CallReason.builder()
                        .createdAt(OffsetDateTime.parse("2026-04-26T18:06:51.940749Z"))
                        .reason("Appointment reminders")
                        .build()
                )
                .certifyBrandIsAccurate(true)
                .certifyIpOwnership(true)
                .certifyNoShaftContent(true)
                .createdAt(OffsetDateTime.parse("2026-04-26T18:06:51.940749Z"))
                .displayName("Acme Plumbing")
                .addDocument(
                    Document.builder()
                        .documentId("2a7e8337-e803-4057-a4ae-26c40eb0bc6c")
                        .documentType(Document.DocumentType.BUSINESS_REGISTRATION)
                        .description("Certificate of incorporation.")
                        .build()
                )
                .enterpriseId("4a6192a4-573d-446d-b3ce-aff9117272a6")
                .expiringAt(OffsetDateTime.parse("2027-04-26T18:07:29.537981Z"))
                .logoUrl("https://acmeplumbing.example.com/logo-256.bmp")
                .rejectedAt(OffsetDateTime.parse("2019-12-27T18:11:19.117Z"))
                .addRejectionReason(
                    RejectionReason.builder()
                        .code("documentation_incomplete")
                        .detail("Provided documents do not establish business identity.")
                        .message("Please re-upload a clearer scan of the certificate.")
                        .title("Documentation incomplete")
                        .build()
                )
                .reselling(false)
                .status(DirStatus.DRAFT)
                .submittedAt(OffsetDateTime.parse("2026-04-26T18:07:03.716411Z"))
                .updatedAt(OffsetDateTime.parse("2026-04-26T18:09:24.785211Z"))
                .verifiedAt(OffsetDateTime.parse("2026-04-26T18:07:29.537926Z"))
                .build()

        val roundtrippedDir =
            jsonMapper.readValue(jsonMapper.writeValueAsString(dir), jacksonTypeRef<Dir>())

        assertThat(roundtrippedDir).isEqualTo(dir)
    }
}
