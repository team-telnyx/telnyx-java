...
class WebhookVerificationTest {
    ...
    fun `verify throws when signature header is missing`() {
        ...
        assertThatThrownBy {
            WebhookVerification.verify("{}", headers, "dGVzdGtleQ==")
        }
    ...
    fun `verify throws when public key is invalid size`() {
        ...
        val headers = mapOf(
            ...
            "telnyx-signature-ed25519" to "QUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUE=",
        ...
        assertThatThrownBy {
            WebhookVerification.verify("{}", headers, invalidKey)
        }
            .isInstanceOf(WebhookVerificationException::class.java)
    ...
    fun `verify throws when signature is invalid size`() {
        ...
        assertThatThrownBy {
            WebhookVerification.verify("{}", headers, validKey)
        }
            .isInstanceOf(WebhookVerificationException::class.java)
    ...
    fun `constants have expected values`() {
        assert(WebhookVerification.SIGNATURE_HEADER == "telnyx-signature-ed25519")
        assert(WebhookVerification.TIMESTAMP_HEADER == "telnyx-timestamp")
        assert(WebhookVerification.TIMESTAMP_TOLERANCE_SECONDS == 300L)
        assert(WebhookVerification.ED25519_PUBLIC_KEY_SIZE == 32)
        assert(WebhookVerification.ED25519_SIGNATURE_SIZE == 64)
    }
}
