...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { TelnyxInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw TelnyxInvalidDataException("`$name` is not set")
            is JsonNull -> throw TelnyxInvalidDataException("`$name` is null")
            else -> throw TelnyxInvalidDataException("`$name` is invalid, received $this")
        }
